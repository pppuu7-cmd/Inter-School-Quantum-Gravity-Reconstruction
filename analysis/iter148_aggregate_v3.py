#!/usr/bin/env python3
"""ITER148 v3 exact aggregator for tau x design-panel chunk shards.

Frozen ITER148 scientific object/predicates are unchanged from preregistration and
v2. v3 only reconstructs each frozen tau solve from smaller exact production
chunks, avoiding long-lived SymPy state and redundant generic Matrix.rank calls.
"""
from __future__ import annotations
import gc,itertools,json,math,sys,urllib.request
...
for p in root.glob('iter148_v3_tau_*_chunk_*.json'):
    o=json.loads(p.read_text()); key=(int(o['tau_index']),int(o['chunk_index']))
    if key in parts: raise SystemExit(f'duplicate chunk {key}')
    parts[key]=o
required=[(j,c) for j in range(9) for c in range(3)]
missing=[x for x in required if x not in parts]
if missing: raise SystemExit(f'missing v3 chunks: {missing}')
...
for j in range(9):
    y_by_panel={}; held_direct=None
    for c in range(3):
        o=parts[(j,c)]; lo=15*c; hi=lo+15
        chunk_integrity_ok &= (o.get('scientific_predicates_changed') is False and int(o.get('basis_size',-1))==45 and int(o.get('design_rank',-1))==45 and int(o.get('panel_start',-1))==lo and int(o.get('panel_stop_exclusive',-1))==hi)
        for row in o.get('design_values',[]):
            idx=int(row['panel_index'])
            if idx in y_by_panel: raise SystemExit(f'duplicate design panel tau={j} panel={idx}')
            y_by_panel[idx]=R(row['value'])
        if c==0: held_direct=o.get('training_heldout_direct',[])
    if sorted(y_by_panel)!=list(range(45)): raise SystemExit(f'tau {j} incomplete design coverage')
    coeff=[s.factor(x) for x in (Ainv*s.Matrix([y_by_panel[i] for i in range(45)]))]
    if held_direct is None or len(held_direct)!=3: raise SystemExit(f'tau {j} missing training heldouts')
    held=[]; held_ok=True
    for row in held_direct:
        bvals=[R(x) for x in row['basis_values']]; direct=R(row['direct'])
        recon=s.factor(sum(c*v for c,v in zip(coeff,bvals))); eq=base.exact_zero(direct-recon); held_ok &= bool(eq)
        held.append({'panel':int(row['panel']),'direct':str(direct),'invariant':str(recon),'equal':bool(eq)})
    tau_shards[j]={'tau_index':j,'tau':str(s.Rational(j,8)),'basis_size':45,'design_rank':45,'coefficients':[str(x) for x in coeff],'training_heldouts':held,'training_heldouts_ok':bool(held_ok)}
    print(json.dumps({'stage':'tau_reconstructed','tau_index':j,'heldouts_ok':held_ok}),flush=True); reset_sympy()
...
for pi,(q0,k0,n0,tv) in enumerate(direct_panels):
    ...
    eq=base.exact_zero(direct-fast); fast_authority &= bool(eq)
    fast_rows.append({'q':list(q0),'k':list(k0),'n':list(n0),'tau':str(tv),'direct':str(direct),'fast':str(fast),'equal':bool(eq)})
    print(json.dumps({'stage':'fast_authority_done','panel':pi,'equal':bool(eq)}),flush=True); del direct,fast; reset_sympy()

t=s.symbols('tau'); labels=[base.basis_label(x) for x in base.BASIS]
coeff_polys=[]; tau_degree_ok=True; max_tau_degree=0
for j,label in enumerate(labels):
    vals=[R(tau_shards[k]['coefficients'][j]) for k in range(9)]; delta=newton_deltas(vals); poly=newton_poly(delta,t)
    dg=s.Poly(poly,t).degree() if poly!=0 else 0; tau_degree_ok &= bool(dg<=8); max_tau_degree=max(max_tau_degree,int(dg)); coeff_polys.append(poly)
source_weight_ok=all(base.exact_zero(p.subs(t,1)) for p in coeff_polys)
train_held_ok=all(bool(tau_shards[j]['training_heldouts_ok']) for j in range(9)); design_ok=bool(chunk_integrity_ok)
...
for tv in (s.Rational(1,3),s.Rational(2,5)):
    ...
    for idx,(q0,k0,nv0) in enumerate(held_seed):
        ...
        eq=base.exact_zero(direct-recon); tau_held_ok &= bool(eq)
        held_tau_rows.append({'tau':str(tv),'panel':idx,'direct':str(direct),'invariant':str(recon),'equal':bool(eq)})
        print(json.dumps({'stage':'no_refit_heldout_done','tau':str(tv),'panel':idx,'equal':bool(eq)}),flush=True); del direct,recon; reset_sympy()

# Frozen routing / permutation / parity controls.
q=(s.Integer(1),2,-1,1); k=(s.Integer(2),-1,1,0); n=(s.Integer(1),0,0,0); tv=s.Rational(1,3)
val,p,r=base.numerator_fast(q,k,n,tv); routing_ok=all(base.exact_zero(p[i]+k[i]+r[i]) for i in range(4)); perm_ok=True
for perm in itertools.permutations(range(3)):
    v,_,_=base.numerator_fast(q,k,n,tv,perm); perm_ok &= bool(base.exact_zero(v-val)); del v; reset_sympy()
flip=base.numerator_fast(tuple(-x for x in q),tuple(-x for x in k),n,tv)[0]; parity_ok=base.exact_zero(val-flip); reset_sympy()
...
A_D2=s.factor(D2.subs({K:L/t**2,S:-T/t,b:-c/t})); A_D3=s.factor(D3.subs({K:L/t**2,S:-T/t,b:-c/t})); mapA_ok=base.exact_zero(A_D2-L/t**2) and base.exact_zero(A_D3-(Q+2*T+L))
B_D1=s.factor(D1.subs({K:L/(1-t)**2,S:T/(1-t),b:c/(1-t)})); B_D2=s.factor(D2.subs({K:L/(1-t)**2,S:T/(1-t),b:c/(1-t)})); mapB_ok=base.exact_zero(B_D1-(Q+2*T+L)) and base.exact_zero(B_D2-L/(1-t)**2)
KC=Q/t**2+2*T/(t**2*(1-t))+L/(t**2*(1-t)**2); SC=Q/t+T/(t*(1-t)); bC=a/t+c/(t*(1-t)); C_D1=s.factor(D1.subs({K:KC,S:SC,b:bC})); C_D3=s.factor(D3.subs({K:KC,S:SC,b:bC})); mapC_ok=base.exact_zero(C_D1-(Q+2*T+L)/t**2) and base.exact_zero(C_D3-L/(1-t)**2)
...
out={
 ...
 'implementation_note':'v3 tau x 15-design-panel chunks; frozen ITER148 object/predicates unchanged; design rank uses exact incremental-pivot certificate from frozen design_panels; SymPy state bounded between independent exact evaluations',
 'scientific_predicates_changed':False,'source_blob':blob,'basis_size':45,'design_rank':45 if design_ok else None,
 'design_rank_certificate':'frozen design_panels accepted 45 exact independent rows for 45 columns','training_taus':[str(s.Rational(j,8)) for j in range(9)],
 'max_tau_degree':max_tau_degree,'coefficient_formulas':formulas,'fast_authority_panels':fast_rows,
 'training_tau_heldouts':{str(j):tau_shards[j]['training_heldouts'] for j in range(9)},'heldout_tau_rows':held_tau_rows,
 'bubble_reduction':{'identity':'tau*D1+(1-tau)*D3-tau*(1-tau)*D2=Q','partial_fraction':['tau/Q * N/(D2*D3)','(1-tau)/Q * N/(D1*D2)','-tau*(1-tau)/Q * N/(D1*D3)'],'canonical_maps':{
  ...
  'C':{'loop_map':'l=(1-tau)*(tau*k-q)','invariants':{'K':'Q/tau^2+2*T/[tau^2(1-tau)]+L/[tau^2(1-tau)^2]','S':'Q/tau+T/[tau(1-tau)]','b':'a/tau+c/[tau(1-tau)]'},'prefactor':'-[tau*(1-tau)]^(3-d)/Q'}},'endpoint_note':'tau=0,1 use ITER145 endpoint degenerations; do not substitute directly into singular canonical maps'},
 'checks':checks,'manual_predicate_L_audit_commit':'1bfea2d5f9ac8baf248a173390c7b9b656c87f21','manual_predicate_L_audit_classification':'MANUAL_PREDICATE_L_PASS_SCOPED',
...
Path('iter148_m3_full_invariant_bubble_reduced_numerator_v3.json').write_text(json.dumps(out,indent=2)+'\n',encoding='utf-8')
