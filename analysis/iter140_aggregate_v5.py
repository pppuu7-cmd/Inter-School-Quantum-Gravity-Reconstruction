#!/usr/bin/env python3
"""ITER140 v5 aggregator: combine exact family x dimension shards."""
...
NAMES=['M_R2_chi1_dR1','M_R1_chi1_dR2','G_R1_chi2_Gamma2_dR1']
requiredD=list(range(3,11)); shards={}
for p in root.glob('iter140_v5_D*.json'):
    o=json.loads(p.read_text()); shards[(int(o['D']),o['family'])]=o
missing=[(D,n) for D in requiredD for n in NAMES if (D,n) not in shards]
if missing: raise SystemExit(f'missing shards: {missing}')

def R(x): return s.sympify(x)
def zero(x): return bool(x==0 or s.cancel(x)==0)
def deltas(vals):
...
def eval_newton(ds,D): return s.factor(sum(ds[j]*math.comb(D-3,j) for j in range(len(ds))))
...
labels=frozen.basis_labels(); d=s.symbols('d'); train=[3,4,5,6,7]; validate=[8,9,10]
formulas={}; DLT={}; trace_ok=True; coeff_ok=True; maxdeg=0
for name in NAMES:
    formulas[name]=[]; DLT[name]=[]
    for j,label in enumerate(labels):
        vals=[s.factor((D-2)**2*R(shards[(D,name)]['coefficients'][j])) for D in train]
        ds=deltas(vals); DLT[name].append(ds); dg=degree(ds); maxdeg=max(maxdeg,dg); trace_ok &= dg<=4
        poly=poly_newton(ds,d); coeff=s.cancel(poly/(d-2)**2)
        for D in validate:
            coeff_ok &= zero(s.factor(eval_newton(ds,D)/(D-2)**2)-R(shards[(D,name)]['coefficients'][j]))
...
held_ok=all(bool(shards[(D,n)]['heldouts_ok']) for D in requiredD for n in NAMES)
d4_ok=all(bool(shards[(4,n)]['D4_terminal_authority_ok']) for n in NAMES)
cont=[]; cont_ok=True
for D in validate:
    row={'D':D}
    for name in NAMES:
        h=shards[(D,name)]['heldouts'][1]; b=[R(x) for x in h['basis_values']]
        cc=[s.factor(eval_newton(DLT[name][j],D)/(D-2)**2) for j in range(len(labels))]
        recon=sum(x*y for x,y in zip(cc,b)); direct=R(h['direct']); eq=zero(recon-direct); cont_ok &= eq
        row[name]={'direct':str(direct),'continued':str(recon),'equal':eq}
    cont.append(row)
checks={
 'A_basis_size_28':len(frozen.BASIS)==28,
 'B_design_rank_28':all(int(shards[(D,n)]['design_rank'])==28 for D in requiredD for n in NAMES),
...
out={'gate':'ITER140_FIXED_GEODESIC_CURVATURE_FIRST_MG_GENERAL_D_INVARIANT_CONTINUATION','classification':cls,
 'implementation_note':'v5 family x dimension exact shards; frozen ITER140 predicates unchanged; D4 cross-authority uses hash-pinned terminal ITER138/139 artifact values',
 'basis_labels':labels,'training_dimensions':train,'validation_dimensions':validate,'max_observed_trace_polynomial_degree':maxdeg,
 'coefficient_formulas':formulas,'continuation_heldouts':cont,
 'D4_cross_authority':{n:shards[(4,n)]['D4_terminal_authority'] for n in NAMES},
...
Path('iter140_first_mg_general_d_invariant_continuation_v5.json').write_text(json.dumps(out,indent=2)+'\n',encoding='utf-8')
